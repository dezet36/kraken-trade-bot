...
def widen_is_inert(min_rr=None):
    """
    Правда ли, что THIN_STOP='widen' сейчас неотличим от 'skip'.

    Расширение стопа до пола даёт rr = half / пол, а расширяем мы ровно тогда,
    когда half < пол. Значит расширенное отношение всегда меньше единицы, и
    порог MIN_RR >= 1.0 отклоняет каждый такой сетап — то есть 'widen'
    заканчивается тем же отказом, что и 'skip'.

    Проверено перебором: при MIN_RR 1.0, 0.8 и 0.7 расширенный сетап
    отклоняется; развилка оживает с 0.625 и ниже.

    Знать это нужно потому, что параметр обещает измеримый выбор. Сравнить
    'widen' со 'skip' при пороге по умолчанию значит сравнить значение само с
    собой и записать в вывод, что разницы нет.
    """
    value = params.MIN_RR if min_rr is None else min_rr
    try:
...
def build_trade(setup, target_frac=None, stop_frac=None, min_rr=None,
    ...
    if distance < floor:
        ...
        #
        # ВЫБОР СУЩЕСТВУЕТ НЕ ВСЕГДА, И ЭТО НАДО ЗНАТЬ. Расширение даёт
        # rr = half / пол, а расширяем мы ровно тогда, когда half < пол —
        # значит расширенное отношение ВСЕГДА меньше 1.0. Ниже стоит проверка
        # `rr < min_rr`, и при MIN_RR >= 1.0 она отклоняет любой расширенный
        # сетап. То есть при настройках по умолчанию (MIN_RR = 1.0) 'widen'
        # неотличим от 'skip', и замер, ради которого выбор выносили в
        # параметр, сравнил бы значение само с собой.
        #
        # Поведение здесь верное — брать сделку с rr ниже единицы не стоит, —
        # а неверным было обещание выбора. Условие названо в widen_is_inert и
