...
# Двенадцать, а не двадцать, с 23.09.2026. Замер по 41 плану: уровни
# L13–L20 попали в поля плана 14 раз из 123 (11%), а L16–L20 — ни разу.
# Хвост таблицы стоил ~370 токенов вопроса и ничего не решал, а место в
# окне понадобилось под счёт модели и порядок разбора.
MAX_LEVELS = 12
...
# Восемь свечей вместо двенадцати: форма последних часов видна и по ним, а
# структура, уровни и размах лежат в своих блоках. Освобождает ~160 токенов.
CANDLES_SHOWN = 8
...
def build(pair, df, at=None, news=None, market=None, history=None):
    ...
    for level in found:
        ...
        # Объём и дельта печатаются, только когда говорят что-то. ×1.0 и 0%
        # занимали место в каждой из двадцати строк и не отличали уровень от
        # уровня; место в окне ушло под счёт модели и порядок разбора.
        vx = level.get('volume_x')
        vol = (f"   объём ×{vx}" if vx and (vx >= 1.3 or vx <= 0.7) else '')
        d = level.get('delta')
    ...
    # Счёт модели — последним блоком, прямо перед вопросом: это про НЕЁ, а
...
def _scoreboard_lines():
    """
    Счёт самой модели: сколько её планов дошло до цели раньше стопа, в
    разрезе стороны и согласия со структурой 4ч.

    ЗАЧЕМ ЭТО В РАЗМЕТКЕ. До 23.09.2026 модель начинала каждый разбор с
    чистого листа и не могла знать, что её планы против тренда 4ч дают два
    попадания из десяти. Замер по 27 сделкам: по тренду +9.17R (47%
    прибыльных), против тренда −2.99R (20%). Ни одно правило в промте этого
    ...
    def pair_text(name, pair_):
        n, won = pair_
        return f'{name} {won}/{n}' if n else ''

    out = ['ТВОЙ СЧЁТ (твои прошлые планы; успех = цель раньше стопа)']
    out.append('  ' + '   '.join(x for x in (
        pair_text('всего', (sb['n'], sb['won'])),
        pair_text('по тренду 4ч', sb['with_trend']),
        pair_text('ПРОТИВ тренда 4ч', sb['against_trend']),
        pair_text('лонги', sb['long']),
        pair_text('шорты', sb['short'])) if x))
    n_cond, fired = sb.get('with_condition') or (0, 0)
    if n_cond:
        out.append(f'  условие входа наступило {fired}/{n_cond} — остальные планы '
                   f'умерли, не дождавшись')
    out.append('  Это твои результаты. Класс планов, который у тебя не работает, '
               'не повторяй без нового довода.')
    out.append('')
    return out
...
def market_lines(market, price_now):
    ...
    # С 23.09.2026 карта весит ОБОРОТОМ свечи, а не приростом ОИ
    # (llm_market.liquidation_estimate). Заголовок говорит модели, ОТКУДА
    # взялось число, и расхождение с расчётом — это ложь в разметке,
    # а не описка: модель строит на нём доводы.
    out += ['', 'КАРТА ЛИКВИДАЦИЙ (ОЦЕНКА по обороту свечей и типичным плечам — '
...
# Насколько должна измениться капитализация USDT за сутки, чтобы считать
# это притоком или оттоком денег, а не шумом округления.
USDT_FLOW_PCT = 0.3


def _macro_verdict(m):
    """
    Что доля стейблкойна говорит — и чего она НЕ говорит.

    ЗАМЕР 23.09.2026 по своему ряду (508 точек за двое суток): изменение
    USDT.D связано с ОДНОВРЕМЕННЫМ ходом рынка на −0.92, а с будущим —
    на −0.04. То есть доля почти не предсказывает, а пересказывает: при
    неизменной капитализации USDT её доля — это просто обратная величина
    капитализации рынка, другая запись фразы «рынок вырос/упал», которую
    модель и так видит в ходе цены.

    Информацию несёт другое: КАПИТАЛИЗАЦИЯ USDT. Растёт она — печатают
    новые стейблы, на рынок пришли деньги, которым ещё предстоит что-то
    купить. Стоит — переложились между активами, нового спроса нет.
    Поэтому фразу строим от притока, а не от доли; долю называем, но
    честно говорим, что она значит.

    ОГОВОРКА: ряд — двое суток растущего рынка. Вывод «не предсказывает»
    может не пережить месяца с падением, и ряд копится дальше.
    """
    if not m:
        return ''
    d24 = m.get('usdt_d_24h')
    cap24 = m.get('usdt_cap_24h')
    if d24 is None:
        return ''
    if cap24 is not None and abs(cap24) >= USDT_FLOW_PCT:
        if cap24 > 0:
            return (f'капитализация USDT {cap24:+.1f}% за сутки — на рынок пришли '
                    f'деньги, спрос ещё не потрачен (доля {d24:+.2f} п.п.)')
        return (f'капитализация USDT {cap24:+.1f}% за сутки — деньги уходят С рынка, '
                f'покупателя меньше (доля {d24:+.2f} п.п.)')
    where = 'вверх' if d24 < 0 else 'вниз'
    return (f'доля стейблов {d24:+.2f} п.п. за сутки при неизменной их '
            f'капитализации — это пересказ того, что рынок сходил {where}, '
            f'а не отдельный довод')
...
def _macro_lines(m):
    ...
    # ВЫВОД ОДНОЙ СТРОКОЙ. Блок цитировался в одном разборе из десяти: двадцать
    # чисел про капитализации модель не читала. Доля стейблкойна — это спрос
    # на выход из риска: растёт — деньги уходят из альтов, и лонг против этого
    # идёт против потока всего рынка, а не одной пары.
    verdict = _macro_verdict(m)
    if verdict:
        out.append(f"  ОРИЕНТИР РЫНКА: {verdict}")
