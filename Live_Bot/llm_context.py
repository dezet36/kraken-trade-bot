...
def _macro_verdict(m):
    """
    Что доля стейблкойна говорит о спросе на риск — одной фразой.

    USDT.D — доля USDT в капитализации рынка. Она растёт, когда продают
    монеты за стейблкойн: деньги выходят из риска, и альты падают почти
    всегда вместе. Падает — обратное. Это фон, а не сигнал: он не открывает
    сделку, но лонг против растущей доли стейблов идёт против потока всего
    рынка, а не одной пары.
    """
    if not m:
        return ''
    ...
    if d24 is None:
        return ''
    if d24 > 0.05:
        who = 'деньги уходят в стейблы — риск продают, альты под давлением'
        side = 'лонг альта идёт против фона'
    elif d24 < -0.05:
        who = 'деньги выходят из стейблов в монеты — спрос на риск'
        side = 'шорт альта идёт против фона'
    else:
        return 'доля стейблов стоит — фон ничего не говорит, решай по паре'
    tail = f', {abs(streak)}-й день подряд' if abs(streak) >= 2 else ''
    return f'USDT.D {d24:+.2f} п.п. за сутки{tail}: {who}; {side}'
