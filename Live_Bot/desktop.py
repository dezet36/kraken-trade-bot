...
# Сколько секунд окно должно прожить, чтобы считаться открытым. Меньше — значит
# браузер вышел сам, ничего не показав: закрыть окно за пару секунд человек не
# успевает, а Chrome с испорченным профилем выходит мгновенно.
WINDOW_ALIVE_SECONDS = 4
...
def _ask_after_browser_close():
    """
    Окно браузера закрыли, а бот ведёт позиции. Спросить больше нечем — окно
    и было интерфейсом, — поэтому спрашиваем своим окном.

    ПОЧЕМУ ЭТОГО НЕ БЫЛО. Диалог `_confirm_close` написан и написан верно, но
    привязан внутри ветки СВОЕГО окна, а приложение открывается браузером —
    эта ветка пробуется первой и почти всегда срабатывает. То есть
    предупреждение не показывалось никогда: 16 августа окно закрыли при шести
    открытых позициях, в журнале «Окно: закрыто пользователем» — и никакого
    вопроса.

    Возврат True — не останавливать, открыть окно заново.

    Отвечать по умолчанию «остановить» нельзя: закрытие окна прекращает
    ведение позиций, а человек, закрывший его случайно, ничего об этом не
    знает. Поэтому спрашиваем, и молчание диалога (не смогли показать)
    трактуем как прежнее поведение — остановиться, — чтобы не оставить бота
    работать вслепую.
    """
    count = _open_positions_count()
    if not count:
    ...
    try:
        ...
        keep = messagebox.askyesno(
            APP_TITLE,
            f'Окно закрыто, а бот ведёт открытых позиций: {count}.\n\n'
            'Стопы стоят на бирже и сработают сами. Но перевод в безубыток, '
            'частичные фиксации и выход по времени делает бот — с остановкой '
            'они прекращаются.\n\n'
            'Продолжить работу и открыть окно снова?',
            parent=root)
        root.destroy()
        return bool(keep)
    except Exception as exc:                       # noqa: BLE001
        log(f'Окно: спросить не вышло ({exc}) — останавливаюсь')
        return False
...
def _open_window(url):
    ...
    if process is not None:
        ...
        if _wait_for_browser_window():
            ...
            # ЗАКРЫТИЕ ОКНА СПРАШИВАЮТ, А НЕ ИСПОЛНЯЮТ МОЛЧА. Оно прекращает
            # ведение позиций, и человек, закрывший его случайно, об этом не
            # знает. Согласился продолжить — открываем окно снова и ждём
            # дальше; отсюда цикл, а не однократное ожидание.
            while True:
                _wait_while_browser_window()
                log('Окно: закрыто пользователем')
                if not _ask_after_browser_close():
                    return
                log('Окно: открываю снова — бота решили не останавливать')
                if _open_app_window(url) is None or not _wait_for_browser_window():
                    log('Окно: снова открыть не вышло — пробую своё окно')
                    break
        else:
...
def shares_our_data(holder):
    """
    Держатель порта работает с ТЕМ ЖЕ каталогом данных, что и мы?

    ЭТО НЕ ТО ЖЕ САМОЕ, ЧТО `is_our_bot`. Тот отвечает «это копия нашего
    приложения» — и правильно делает, потому что решает, можно ли её закрывать.
    Но для вопроса «можно ли просто уйти на соседний порт» нужен другой ответ:
    опасны не две копии вообще, а две копии на ОДНИХ ФАЙЛАХ.

    ОТКУДА ЭТО. Человек получил окно «Порт 8787 занят: python.exe (PID 19520).
    Прежнюю копию закрыть не удалось — снимите её вручную». То есть бот
    оказался в состоянии, из которого его вытаскивают через диспетчер задач, —
    ровно то, чего здесь быть не должно.

    Отказ был необоснован. К этому месту замок нашего каталога данных УЖЕ У
    НАС: `single_instance.acquire` отработал выше, и без него до проверки порта
    дело не доходит вовсе. Значит держатель порта с нашими файлами не работает,
    и уход на соседний порт ничего не затирает. А код отказывался переходить
    именно на этом основании — «две копии на одних файлах затрут работу друг
    друга», — хотя условие уже было исключено.

    Проверка поэтому ПОЛОЖИТЕЛЬНАЯ: остаёмся на месте, только если держатель —
    та самая копия, которая записала себя в наш running_app.json, и она жива.
    Мьютекс живёт в пространстве имён сеанса, поэтому копия из другого сеанса
    Windows его не увидит; эта проверка ловит и такой случай, потому что
    running_app.json один на каталог данных.
    """
    pid = (holder or {}).get('pid')
    if not pid:
        return False
    recorded = our_recorded_pid()
    if not recorded or str(pid) != str(recorded):
        return False
    try:
        import single_instance
        return single_instance._alive(int(recorded))
    except Exception:                              # noqa: BLE001
        return True                                # не убедились — не рискуем
...
def main():
    ...
    if not got_lock:
        ...
        # В ЖУРНАЛ — ОБЯЗАТЕЛЬНО. Проверено запуском второй копии: она
        # правильно не берёт порт и не торгует, но в журнале о ней НИ СТРОКИ —
        # только загрузка настроек и тишина. Диалог человек закроет и забудет,
        # а «почему второй запуск ничего не сделал» выясняется потом и уже без
        # диалога. Пишем до окна: окно модальное и ждёт щелчка.
        log(f'вторая копия остановлена замком: уже работает {running} '
            f'(PID {info.get("pid") or "?"}, {where})')
    ...
    if port_busy(config.DASHBOARD_PORT):
        ...
        if freed:
            ...
            # пойдёт.
            #
            # ОСТАНАВЛИВАЕМСЯ ТОЛЬКО РАДИ ОБЩИХ ФАЙЛОВ: две копии на одних
            # данных затрут работу друг друга, и это хуже, чем незапуск. Но
            # здесь стояло `0 if ours` — то есть в диспетчер задач человека
            # отправляла ЛЮБАЯ наша копия, даже с другим каталогом данных, где
            # затирать нечего. См. shares_our_data.
            moved = 0 if shares_our_data(holder) else free_port(
                config.DASHBOARD_PORT + 1)
            if not moved:
                _alert(APP_TITLE, busy + '\n\n' + (
                    'Прежнюю копию закрыть не удалось, а работает она с теми\n'
                    'же данными — вместе они затрут журнал сделок. Снимите\n'
                    'её вручную (диспетчер задач → Подробности) и запустите\n'
                    'снова.'
                    if shares_our_data(holder) else
                ...
                return
            log(f'порт {config.DASHBOARD_PORT} занят ({who}) — '
                f'перехожу на {moved}')
            os.environ['DASHBOARD_PORT'] = str(moved)
            config.DASHBOARD_PORT = moved
            # «Это не наш бот» говорим только когда это правда: порт могла
            # держать и наша копия — с другим каталогом данных.
            whose = ('Это не наш бот, и закрывать его я не стану.'
                     if not ours else
                     'Это другая копия бота, со своими данными —\n'
                     'работать вместе они могут.')
            _alert(APP_TITLE,
                   busy + '\n\n' + whose + '\n\n'
