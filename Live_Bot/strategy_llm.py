...
QUICK_GATES = llm_decide.NO_MODEL_GATES
BATCH = 3
...
def _decide_one(pair, df, market):
    ...
    try:
        ...
        # о размере вопроса, по которому меряется окно контекста.
        stats = llm_local.last_stats() if verdict.get('gate') not in QUICK_GATES else {}
...
def _reshape(pair, verdict, df=None):
    ...
    params = {
        ...
        # сделкам довод не подтвердил: четыре убыточные побывали в плюсе на
        # 1.2–2.7R (COTI +2.71, LINK +2.24, ARB +1.32, SUI +1.21) и вернулись
        # к стопу, частичной фиксации не случилось ни разу. С безубытком на
        # +1R те же сделки дают −3.30R вместо −7.55R.
        'be_level': (verdict['entry'] + abs(verdict['entry'] - verdict['stop'])
                     if verdict['side'] == 'LONG'
                     else verdict['entry'] - abs(verdict['entry'] - verdict['stop'])),
...
def _fractions(count):
    """
    Доли выхода по целям: половина на первой.

    Ближняя цель надёжнее дальней, и именно до неё рынок доходит: по замеру
    22.09.2026 первая цель у ИИ стояла в 2–9.7R, а ход рынка в сделке был
    1.2–2.7R. Половина позиции, снятая на первой цели, превращает такой ход
    в деньги, а не в возврат к стопу.
    """
    if count <= 1:
        return [1.0]
    if count == 2:
