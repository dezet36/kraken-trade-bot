...
def _apply_book(payload):
    ...
    ПОРЯДОК УРОВНЕЙ ОБЯЗАН БЫТЬ «ЛУЧШИЙ ПЕРВЫМ», И ЭТО НЕ УКРАШЕНИЕ.

    Здесь была ошибка, которая ставила заявки по цене в один цент на рынке в
    сорок три. Площадка присылает биды по ВОЗРАСТАНИЮ, а `book.top` берёт
    просто первый уровень — так устроен опрос, который сортирует сам. Снимок из
    потока не сортировался, и лучшим бидом оказывался худший:

        снимок «Theo FDV», 16 уровней: первый бид 0.01, настоящий лучший 0.39
        снимок «CDU Berlin», 56 уровней: первый бид 0.01, настоящий 0.66

    Обе заявки ушли на биржу по 0.01 и стояли там за очередью в двадцать пять
    тысяч контрактов — то есть не стояли вовсе.

    Прошлая правка потока чинила ДЕЛЬТЫ и про снимок молчала, поэтому ошибка
    вернулась тем же путём, каким уходила.
    """
