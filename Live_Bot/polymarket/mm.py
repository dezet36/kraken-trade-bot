...
def adopt_exchange_positions(maker, catalogue=None):
    ...
    for ours, row in net.items():
        ...
        #
        # Когда она открылась на самом деле, узнать неоткуда: биржа отдаёт
        # размер и среднюю цену, но не время. Поставить отсчёт с нуля значило
        # бы дать ей ещё сутки покоя — а она и так лежала без присмотра, и
        # именно это привело к жалобе «весь бюджет завис».
        #
        # Первый срок означает не сброс позиции, а всего лишь отказ от шага
        # внутрь спреда: встаём лучшей ценой и ждём. Риск такой уступки мал,
        # а простоя капитала — велик, и выбор здесь очевиден.
        # Минута сверх срока, а не ровно срок: сравнение строгое, и «ровно»
        # означало бы «ещё не пора» — с точностью до одного такта.
        fresh['opened_ts'] = (fresh.get('opened_ts') or engine._now()
                              - params.MM_MAX_HOLD_HOURS * 3600 - 60)
    ...
    # СХЛОПНУТАЯ ПАРА ИСЧЕЗАЕТ ИЗ ЗАПАСА, ИНАЧЕ ДЕНЬГИ ПОСЧИТАЮТСЯ ДВАЖДЫ.
    #
    # Пару можно погасить в залог — человек делает это в интерфейсе площадки.
    # Тогда токены с биржи уходят, а деньги приходят наличными. Если запись о
    # паре при этом останется, учёт сложит вернувшиеся деньги со стоимостью
    # пары, которой уже нет. Замерено ровно так: после схлопывания двух пар
    # панель показала капитал $50.59 при настоящих $40.59 — ровно на $10 больше.
    #
    # Позиция у такой записи НУЛЕВАЯ, поэтому обычная чистка призраков её не
    # трогала: она смотрит только на позицию.
    held = set(net)
    for token in list(maker.state['books']):
        slot = maker.state['books'][token]
        if slot.get('pair') and str(token) not in held:
            slot['pair'] = 0.0
...
def step(maker, markets, live=False, day_loss=0.0, deadline=None,
    ...
    desperate = set(maker.stale_positions(
        params.MM_MAX_HOLD_HOURS * params.MM_DESPERATE_AFTER))
    ...
    for token, market in by_token.items():
        ...
        if committed + need > budget:
            if slot['position']:
                ...
                # НЕОТПРАВЛЕННАЯ КОТИРОВКА СНИМАЕТСЯ, А НЕ ОСТАЁТСЯ ПРИЗРАКОМ.
                #
                # Пропуская рынок по деньгам, мы к нему в этом такте больше не
                # вернёмся — а вместе с ним не вернёмся и к заявке, которую
                # успели сочинить раньше. Она остаётся в наших книгах без
                # биржевого номера и без ошибки: на бирже её нет, отправлять
                # некому, причины никто не записал.
                #
                # Панель показывала такую строку как «только расчёт» с
                # подсказкой «живой режим выключен» — при включённом живом
                # режиме. Замерено: девять строк из тридцати одной, и по каждой
                # человек искал поломку там, где её не было.
                #
                # Заявку с номером не трогаем: она стоит на бирже и живёт своей
                # жизнью независимо от того, хватает ли нам денег на новые.
                for side in ('bid', 'ask'):
                    order = (slot.get('orders') or {}).get(side)
                    if order and not order.get('live_id'):
