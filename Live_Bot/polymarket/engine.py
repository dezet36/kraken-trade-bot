...
class PaperMaker:
    ...
    def mark_to_market(self, marks):
        ...
        for token, slot in self.state['books'].items():
            # ПАРА «ДА+НЕТ» СТОИТ РОВНО ДОЛЛАР ЗА ШТУКУ, И ОЦЕНИВАТЬ ЕЁ НЕ НАДО.
            #
            # Один контракт погасится единицей, другой нулём — при любом исходе
            # вместе они дают доллар. По СТАВКЕ такая пара в нуле, и знаковая
            # позиция её уничтожает: +5 и −5 дают ноль. Деньги при этом заняты.
            #
            # Замерено на живом счёте: биржа насчитала $40.52, панель показывала
            # $30.77 — ровно две пары по пять контрактов, $10 капитала, которого
            # учёт не видел.
            inventory += float(slot.get('pair') or 0)
    ...
    def day_loss(self, marks=None):
        """
        Убыток за ТЕКУЩИЕ СУТКИ. Ноль, если день в плюсе.

        ЗДЕСЬ БЫЛА ОСТАНОВКА, КОТОРАЯ НЕ ОТПУСКАЛА. Дневным убытком считался
        общий итог с начала работы: `bankroll − (деньги + запас)`. Такая мера
        не обнуляется никогда, и просадка в 5% останавливала торговлю НАВСЕГДА
        — не до завтра, а до ручного вмешательства.

        Поймано на живом счёте: «дневной убыток 7.79 достиг предела 2.00»,
        четырнадцать заявок не отправлено, ноль на бирже. При этом настоящий
        итог был +$0.84: число выросло из разовой правки учёта, а не из
        торговли. Мера, которую двигает исправление бухгалтерии, не годится в
        предохранители.

        Считаем от капитала на начало суток. Точка отсчёта живёт в состоянии и
        переносится сама, когда сутки сменились.
        """
        now = _stamp()[:10]                              # YYYY-MM-DD по UTC
        equity = self.mark_to_market(marks or {})['equity']
        anchor = self.state.get('day_anchor') or {}
        if anchor.get('day') != now:
            # НОВЫЕ СУТКИ — НОВАЯ ТОЧКА ОТСЧЁТА. Первый вызов за день ставит
            # её по текущему капиталу: вчерашние потери сегодня не считаются.
            anchor = {'day': now, 'equity': equity}
            self.state['day_anchor'] = anchor
        return max(0.0, float(anchor.get('equity') or equity) - equity)
