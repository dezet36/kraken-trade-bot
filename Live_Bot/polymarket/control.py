...
VERDICTS = ('нет книги', 'без заявки', 'за очередью',
...
def review(maker, books, catalogue=None, orders=None, now=None):
    ...
    for token, slot in (maker.state.get('books') or {}).items():
        ...
        # ОЧЕРЕДЬ — ЕДИНСТВЕННАЯ ЧЕСТНАЯ МЕРА ИСПОЛНИМОСТИ, и считается она
        # всегда. Первая версия объявляла «вне рынка» всё, что стоит хуже
        # лучшей цены, — и это была ошибка: продажа дороже лучшего аска вполне
        # исполнима, просто после тех, кто дешевле. Разница не в возможности, а
        # в длине очереди, и подменять число словом здесь нельзя.
        ahead = _queue_ahead(live, side, ours, row.get('order_size'))
        row['queue_ahead'] = round(ahead, 1)
        # Хуже лучшей цены — не приговор, но и не мелочь: покупатель берёт
        # дешёвое первым, и вся разница уходит в ожидание.
        row['worse_than_best'] = round(
            (ours - top['ask']) if side == 'ask' else (top['bid'] - ours), 4)

        many = float(row.get('order_size') or 5) * 20
        if ahead <= 0:
            row.update({'verdict': 'первые в очереди',
                        'why': 'исполнят следующей встречной сделкой'})
        elif ahead > many:
            # ТРИДЦАТЬ ЦЕНТОВ ОТ СЕРЕДИНЫ — ЭТО НЕ ОЧЕРЕДЬ, А ДРУГАЯ ЦЕНА.
            # Замерено: держим по 0.637, продаём по 0.619 при рынке 0.329 —
            # порог «не продавать ниже себестоимости» не даёт подвинуться, и
            # заявка стоит там, где не торгуют.
            row.update({
                'verdict': 'за очередью',
                'why': (f'впереди {ahead:.0f} контрактов, и цена на '
                        f'{row["from_mid"]:.3f} от середины'
                        if row['from_mid'] > 4 * (top['spread'] or 0.01)
                        else f'впереди {ahead:.0f} контрактов')})
        else:
