...
# Кэш живёт ДОЛЬШЕ такта: греет его торговый поток, а панель только читает.
# Прежние двадцать секунд означали, что панель регулярно попадает на холодный
# кэш и сама идёт в сеть — с секундами ожидания и случайными таймаутами.
_EXCHANGE_TTL = 90
