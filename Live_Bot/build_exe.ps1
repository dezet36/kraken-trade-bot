...
# шага, упирается в блокировку и возвращает код 1. Времени на это уходит
# столько же, сколько на удачную сборку, а причина в выводе теряется среди
# сотен строк INFO.
#
# Так и вышло: приложение было запущено поверх идущей сборки, пять минут
# работы ушли впустую, и понять почему удалось только по последним строкам
# лога. Отказ должен приходить сразу и называть причину.
$running = @(Get-Process Kraken -ErrorAction SilentlyContinue)
if ($running.Count -gt 0) {
    $ids = ($running | ForEach-Object { $_.Id }) -join ', '
    throw ("Kraken.exe запущен (pid $ids) и держит свой файл — сборка не " +
           "сможет его переписать. Закрой приложение и повтори.")
}
