...
class TestMarkup:
    ...
    # Знаков на токен — НИЖНЯЯ граница по живым вопросам. Замер 25.09.2026 на
    # сервере, 31 свежий разбор: 2.462 / 2.503 / 2.629 (мин / медиана / макс).
    # Здесь стояло 2.3 — и при этом разметка собиралась БЕЗ блока прошлых
    # разборов, который есть в 28 вопросах из 31: одна неточность прикрывала
    # другую. Теперь блок в разметке, а граница — замеренная.
    CHARS_PER_TOKEN = 2.45

    def test_the_markup_still_fits_the_context_window(self, monkeypatch):
        """
        Полный снимок добавляет к вопросу текст, и он обязан помещаться.

        Вместе с блоком прошлых разборов (две строки по 160 знаков) и их
        ...
        history = [{'pair': 'BTCUSDT', 'at': '2026-09-25T10:11:47+00:00', 'decision': 'enter',
                    'side': 'SHORT', 'why': why},
                   {'pair': 'BTCUSDT', 'at': '2026-09-25T08:11:47+00:00', 'decision': 'skip',
                    'gate': 'мало конфлюенса', 'why': why}]
        observed = [{'at': '2026-09-25T10:14:47+00:00', 'side': 'SHORT', 'pct': -12.34,
                     'max_up': 10.4, 'max_down': -16.6, 'hit_tp1': True, 'hit_sl': True, 'hours': 44.0},
                    {'at': '2026-09-25T08:14:47+00:00', 'side': '', 'pct': -12.34,
                     'max_up': 10.4, 'max_down': -16.6, 'hit_tp1': False, 'hit_sl': False, 'hours': 46.0}]
        monkeypatch.setattr(llm_outcomes, 'recent', lambda pair, limit=2: observed)
        text = llm_context.build('BTCUSDT', make_df(wavy(400)),
                                 market=_full_market(), history=history)['text']
        assert 'с тех пор -12.3%' in text, 'блок прошлых разборов не попал в разметку'
        prompt = llm_prompt.build(text)
        # Порог 0.95, а не 0.9: замер на живой разметке ETH 23.09.2026 дал
        # вопрос 9233 токена при окне 12288, то есть настоящий запас — пять
        # процентов, и десятипроцентный порог здесь был не замером, а
        # пожеланием. Синтетика в этом наборе короче живой разметки, поэтому
        # порог должен быть настоящим, иначе тест зелёный там, где сервер
        # уже на грани.
        tokens = len(prompt) / self.CHARS_PER_TOKEN
        need = tokens + config.LLM_MAX_TOKENS
        assert need < config.LLM_CTX * 0.95, (
            f'вопрос ~{tokens:.0f} токенов + ответ '
