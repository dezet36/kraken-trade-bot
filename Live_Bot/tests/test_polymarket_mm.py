...
class TestQuoting:
    ...
    def test_narrow_spread_joins_the_touch_instead_of_being_declined(self):
        """
        При узком спреде встаём НА лучшую цену, а не отказываемся от рынка.

        Прежде этот тест требовал отказа, и требование было ошибочным: оно
        стоило нам лучших рынков биржи. Очередь важна не длиной, а ВРЕМЕНЕМ.
        Замер по 53 рынкам показал, что у шести ожидание меньше часа при
        медиане 23 минуты, и почти все они однотиковые. «Will Alexandria
        Ocasio-Cortez win...»: очередь 9 контрактов при потоке 530 в час —
        одна минута. Такой рынок мы выбрасывали.

        Время считает ОТБОР и сюда доводит только рынки, укладывающиеся в
        предел. Стратегии остаётся выбрать, как встать.
        """
        for bid, ask in ((0.20, 0.21), (0.20, 0.22)):
            q = strategy.desired_quote(_top(bid, ask), _market(tick=0.01), 0, 300)
            assert not q.get('reason'), 'узкий спред больше не отказ'
            assert q['bid'] <= bid + 1e-9, 'встаём на лучшую цену, не внутрь'
            assert q['ask'] >= ask - 1e-9
            assert q['bid'] < q['ask']
