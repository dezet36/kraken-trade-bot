...
def _called():
    ...
    # ДЕФИС ПЕРЕД ИМЕНЕМ ТОЖЕ ЗАПРЕЩЁН. Без него `:nth-child(4)` в стилях
    # читалось как вызов функции `child`, и проверка требовала объявить её в
    # скрипте. Имя JS дефис содержать не может, так что запрет ничего не
    # прячет — зато снимает ложную тревогу на любом селекторе CSS.
