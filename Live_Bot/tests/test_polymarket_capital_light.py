...
ЧАСТЬ ЭТОГО ЗАМЕРА ОКАЗАЛАСЬ ЛОВУШКОЙ, И ЦЕНУ ЕЙ МЫ УЗНАЛИ ПО ВЫПЛАТЕ.

Дешевле всего стоят рынки по 0.02-0.10 — и как раз на них награда за одну
сторону равна нулю: зачётные очки вне полосы 0.10..0.90 считаются как
min(Q₁, Q₂), без деления на три. Живой счёт стоял двадцатью котировками, ни
одной двусторонней, ценами 0.036, 0.041, 0.05, 0.074 — и получил за сутки
восемь центов на все сорок долларов.

Вход дешёвой стороной остаётся, но только внутри полосы. Первая строка замера —
136 рынков по цене 0.02-0.10 за $0.28 — больше не считается возможностью: это
рынки, где мы стояли бы бесплатно.
"""
...
class TestWeEnterOnTheCheapSide:
    ...
    def test_a_cheap_market_is_entered_by_buying(self):
        """Контракт по 0.15 стоит пятнадцать центов; встречный — восемьдесят пять."""
        quote = _quote(0.15)
    ...
    def test_outside_the_paying_band_both_sides_stand(self):
        """
        ГРАНИЦА ДЕШЁВОГО ВХОДА — ПРАВИЛО ПЛОЩАДКИ, А НЕ НАША ОСТОРОЖНОСТЬ.

        Зачётные очки награды равны max(min(Q₁,Q₂), max(Q₁,Q₂)/3) только при
        середине от 0.10 до 0.90. Вне полосы зачёт равен min(Q₁,Q₂), и одна
        сторона даёт РОВНО НОЛЬ.

        Замерено на живом счёте: двадцать котировок, ни одной двусторонней,
        цены 0.036, 0.041, 0.05, 0.074 — и восемь центов награды за сутки на
        весь счёт. Дешёвая сторона дешёвого рынка это та, за которую не платят.
        """
        assert _quote(0.06)['only'] is None
        assert _quote(0.96)['only'] is None
