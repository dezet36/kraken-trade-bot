...
# Сколько СВОИХ денег стратегии позволено держать под риском одновременно.
# Порог на стратегию, а не на сумму: депозиты раздельные и не складываются.
LIMIT_OWN_PCT = 20.0


def check_risk():
    """
    Риск КАЖДОЙ стратегии в её собственном депозите.
    ...
    диагностика этого не видела.

    Потом ошибка сменилась на противоположную: проценты стали складывать по
    всем стратегиям. Но депозит у каждой СВОЙ и в деньгах ($20 000 у фибо,
    $4 000 у боллинджера), и сумма их процентов не относится ни к одному из
    них. Диагностика показывала «до 40% депозита под риском одновременно» —
    число, которого нет.
    ...
    # КАЖДАЯ СТРАТЕГИЯ СУДИТСЯ В СВОИХ РАМКАХ. Здесь складывались проценты
    # разных депозитов и сумма называлась «процент депозита» — а депозит у
    # каждой свой и в деньгах. Подробности в risk_gate.exposure_by_strategy.
    worst = risk_gate.exposure_by_strategy([(n, s, r) for n, s, r in rows])
    unbounded = [n for n, pct in worst if pct is None]
    heavy = [(n, pct) for n, pct in worst if pct is not None and pct > LIMIT_OWN_PCT]
    spread = ', '.join(f'{n} до {pct:g}% своего' if pct is not None
                       else f'{n} без предела'
                       for n, pct in worst)

    if unbounded:
        return _result(WARN,
                       f'Без предела позиций: {", ".join(unbounded)}',
                       f'слотов не ограничено — стратегия наберёт сколько '
                       f'найдёт ({spread})',
                       'задайте число слотов этим стратегиям')
    if heavy:
        names = ', '.join(f'{n} {pct:g}%' for n, pct in heavy)
        return _result(WARN, f'Много риска в своём депозите: {names}',
                       spread,
                       f'у стратегии под риском больше {LIMIT_OWN_PCT:g}% её '
                       f'собственных денег — уменьшите слоты или риск на сделку')
    return _result(OK, 'Риск в рамках каждой стратегии', spread)
