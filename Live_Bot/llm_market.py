...
# Набор плеч совпадает с Model 1 у CoinGlass (10/25/50/100) — это выяснилось
# при разборе их документации 23.09.2026, независимо друг от друга. Их
# Model 2 добавляет малые плечи (2x/3x/5x), и мы это проверили: карта стала
# ХУЖЕ (поймано 41.4% против 52.7%, превышение 2.58 против 2.84). Причина
# арифметическая: при плече 5 ликвидация в 20% от цены, при 2 — в 50%, и за
# те часы, на которые строится план, туда не доходят. Малые плечи не
# добавлять.
...
# Меньше этого числа свечей с живым ОИ — карты нет вовсе.
#
# Пока весом служил ПРИРОСТ ОИ, устаревший ряд отсеивался сам: у стоячего
# ряда прироста нет, и ни одна свеча не проходила. С весом по обороту это
# перестало работать — ряд, в котором свежими оказались две записи, давал
# «карту» из одной свечи. Правило проекта требует прочерка, а не оценки,
# построенной на том, что успел записать сборщик до простоя.
LIQ_MIN_BARS = 5


def liquidation_estimate(pair, df, index, upto=None, bars=None):
    """
    Где скопились ликвидации ОЦЕНОЧНО — по обороту свечей и типичным плечам.

    Позиции, открытые на свече с ценой P при плече L, ликвидируются у
    P·(1 − 1/L) для лонгов и P·(1 + 1/L) для шортов. Оборот свечи — сколько
    таких позиций могло добавиться; куда именно, лонги или шорты, — по
    направлению свечи (см. oi_flow). Уровни, которые цена уже прошла,
    выброшены: те позиции ликвидированы.

    Вес — оборот, а не прирост ОИ (с 23.09.2026, по замеру — см. тело
    функции). Так же считает CoinGlass: «trading volume, leverage ratios,
    ...
    longs, shorts, used = [], [], 0
    for (_, oi_prev, px_prev), (i, oi, px) in zip(series, series[1:]):
        # Свеча без СВЕЖЕГО ОИ не считается вовсе: ряд позиционирования может
        # отстать или умереть, и тогда карты нет — прочерк, а не тихо построенная
        # по одним свечам оценка. ОИ больше не вес, но остаётся условием живости
        # данных (правило проекта: внешнее число старше предела — прочерк,
        # не последнее известное).
        if oi is None or oi_prev is None or px_prev <= 0:
            continue
        # ВЕС — ОБОРОТ СВЕЧИ, А НЕ ПРИРОСТ ОИ, и считается по ВСЕМ свечам.
        #
        # До 23.09.2026 брались только свечи, где открытый интерес вырос, а
        # вес равнялся этому приросту. Остальные выбрасывались целиком —
        # отсюда карта шириной 7.8% ценовой полосы, молчавшая в 61% случаев;
        # модель ссылалась на неё лишь в 17% разборов, и не по глупости.
        #
        # Замер 23.09.2026 (322 точки, 10 пар, четверо суток; карта строится
        # в момент t, сверяется с настоящими ликвидациями следующих 4 часов):
        #     прирост ОИ   поймано 34.7%, покрытие 7.8%,  молчит 61%
        #     оборот свечи поймано 52.7%, покрытие 18.5%, молчит 43%
        # Оборот выиграл у ВСЕХ ДЕСЯТИ пар без исключения — от BTC
        # (44.3→57.8%) до SOL (28.5→74.6%). Превышение над случайностью
        # снизилось с 4.43 до 2.84: карта стала шире и оттого менее
        # «точечной», но её бедой была не точность, а молчание.
        #
        # Позиции открываются на каждой свече, где шла торговля, а не только
        # там, где ОИ вырос: рост ОИ — это ЧИСТЫЙ прирост, в нём взаимно
        # уничтожаются открытия и закрытия одного часа. Оборот же виден
        # всегда. ОИ остаётся условием существования карты (ряд нужен, чтобы
        # знать, что данные живы), но весом больше не служит.
        opened = float(volume[i]) * px
        if opened <= 0:
            continue
        used += 1
...
def _macro_facts(upto=None):
    """
    Рынок в целом на момент upto (мс) — конец ТОЙ свечи, а не её открытие.

    upto в снимке — время открытия последней свечи; точки ряда за текущий
    час позже него, и с отсечкой «не позже открытия» блок был пустым при
    живом ряде (21.09.2026, первый разбор после выкатки). Для живой свечи
