...
ВНИМАНИЕ (25.09.2026): ЧИСЛА НИЖЕ ПОЛУЧЕНЫ С ЗАГЛЯДЫВАНИЕМ ВПЕРЁД. Заявка
рождается на ОТКРЫТИИ часовой свечи сигнала (created = window.iloc[-1]), а
сигнал считается по этой свече целиком — лимит может налиться ценой, которая
сигнал и породила. В research/backtest_smc.py то же давало фибо 27–29% сделок
и 64–70% прибыли; с заявкой на закрытии свечи и правилами живого брокера фибо
там −3.8R (2025–26) и −39.1R (2022–23), см. results/broker_rules_fibo_*.
Выбор глубины входа ниже требует перемера с заявкой на закрытии свечи.
