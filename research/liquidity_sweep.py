...
РЕЗУЛЬТАТ: 28 КЛЕТОК, ВСЕ ОТРИЦАТЕЛЬНЫЕ. Семь вариантов на четырёх периодах,
просадки 63-93%, 2869 сделок в базовом варианте, интервалы ±0.06 — данных
достаточно, и это не «мало наблюдений».

ПОЧЕМУ НЕ РАБОТАЕТ, ВИДНО ТОЧНО. Геометрия вышла ровно как задумано: отношение
риска к прибыли 2.1 при доле попаданий 33.1%. Безубыток при RR 2.1 требует
32.3% — то есть сетап попадает ТОЧНО В БЕЗУБЫТОК, а издержки 0.096 R уводят
его в минус. Валовый край базового варианта +0.012, то есть ноль.

ЗАМЫСЕЛ ПРО ЦЕЛЬ ПОДТВЕРДИЛСЯ, И ЭТО СТОИТ ЗАПИСАТЬ ОТДЕЛЬНО. Цель ЗА
скоплением лучше цели НА нём: валовый край +0.012 против +0.009, устойчиво.
Рассуждение верное — цена действительно тянется чуть дальше черты, а не к ней.
Но прибавка 0.003 R при издержках 0.096 ничего не решает: уточнять цель у
входа, не имеющего края, — значит уточнять ноль.

ТРЕТИЙ ВНЕШНИЙ ИСТОЧНИК, НЕ ВОСПРОИЗВЕДЁННЫЙ НА НАШИХ ДАННЫХ. Фильтр «только у
сброса фандинга» оказался ХУДШИМ вариантом: −0.134 против −0.084 у базы, а на
одном периоде −0.198. Выносов в этом окне действительно меньше (7118 против
36889), но качество их хуже, а не лучше. Счёт за два дня:

    утверждение                          обещано              получено
    пересыхание объёма перед пробоем     65% против 48%       +0.005 R
    ADX как фильтр для Боллинджера       58-65% против 45%    не двигает вовсе
    выносы у сброса фандинга             учащаются            хуже на 0.05 R

ЧТО ОСТАЛОСЬ НЕПРОВЕРЕННЫМ. Дельта продавца — четвёртый компонент замысла. Её
нельзя измерить на истории, сбор запущен. Ожидание честное: валовый край равен
нулю при 2869 сделках, и это не «немного не хватает подтверждения», а
отсутствие направленной информации во входе. Дельта могла бы отбирать лучшие
выносы из имеющихся — но отбирать надо из чего-то. Проверить это всё равно
нужно, когда данные накопятся: утверждение «дельта не поможет» тоже подлежит
измерению, а не объявлению.
