...
ИТОГ: ОТВЕРГНУТО. Из девяти вариантов приёмку по знаку прошёл ровно один —
«пробой глубже 0.30 ATR»: +0.096, +0.042, +0.239, +0.169 при 494-712 сделках,
интервал от нуля на двух периодах. По просадке он не прошёл (36.4% на 2022 при
пороге 30), и это оказалось не главным: просадка в процентах депозита —
свойство размера ставки, а не стратегии. При 0.25% на сделку она падает до
20.1 / 9.6 / 8.6 / 13.1, а R на сделку не меняется вовсе, потому что нормирован
на риск.

Убило другое. Девять вариантов при четырёх периодах дают 9/16 ≈ 0.56 ожидаемых
«плюс на всех четырёх» просто от случайности — то есть ровно один, который и
нашёлся. Проверка на отложенных парах (research/trend_holdout.py) это
подтвердила: на половине, которой отбор не видел, получилось +0.002, -0.045,
+0.151, +0.315. Два периода из четырёх — ноль и минус.

Четвёртый независимый замер против этого жанра, и первый, где отказ пришёл не
от издержек, а от подгонки. Оговорка про девять вариантов была записана ДО
проверки и оказалась не перестраховкой.

Что остаётся правдой и после отказа: валовый край положителен во всех клетках
всех вариантов, а короткие каналы (двое и четверо суток) проваливаются, тогда
как недельный держится. Согласуется с тремя прежними замерами о возврате после
короткого ухода за уровень. Почему свечных признаков не хватает, чтобы это
превратить в край, — измерено отдельно: research/break_diagnosis.py.
...
# ИСПОЛНЕНИЕ НА ПЯТИМИНУТКАХ, И ЭТО НЕ ДЕТАЛЬ. Вход здесь по типу «стоп»:
# заявка срабатывает при уходе цены за границу канала. На ЧАСОВЫХ свечах такой
# вход не может налиться хуже заявки по построению — чтобы часовая свеча
# дотянулась до цены, она обязана была открыться по эту сторону от неё, и
# проверка на разрыв не срабатывает никогда.
#
# Замерено на уровнях, у которых вход того же типа: на часе средний вход хуже
# заявки на +0.000% при максимуме −0.000%, на пятиминутках +0.280% при
# максимуме +4.333%. Разница съедала около 40% результата.
#
# Первый прогон этого замера шёл на часовом исполнении и потому завышал край.
EXEC_TIMEFRAME = '5m'
...
def scan(cache_dir, pairs, label):
    ...
    for pair in pairs[:PAIRS_LIMIT]:
        ...
        if loaded is None or '1h' not in loaded:
            continue
        df = loaded['1h']
        if EXEC_TIMEFRAME not in loaded:
            continue
        data[pair] = loaded[EXEC_TIMEFRAME]
