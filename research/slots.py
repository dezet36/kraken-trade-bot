...
РЕЗУЛЬТАТ: НИ ОДИН ПРЕДЕЛ НЕ ПРИНЯТ, но замер ответил на другой вопрос — тот,
ради которого его и стоило гонять.

У УРОВНЕЙ И SMC ПРЕДЕЛ НЕ ЗНАЧИТ НИЧЕГО. Обе насыщаются: уровни на восьми
слотах, SMC на пяти. Дальше числа не меняются вообще — они просто не находят
больше одновременных сетапов. Снятие предела им ничего не стоило.

У ФИБОНАЧЧИ ПРЕДЕЛ ЗНАЧИТ ОЧЕНЬ МНОГО, и снятие ему повредило:

    предел   сделок (бык)  R/сделку  сумма R  просадка  R/просадку
    3            1253        0.097     122      12.0%      10.1
    5            2094        0.134     280      14.7%      19.0
    8            3204        0.093     298      19.3%      15.4
    12           4527        0.070     316      20.9%      15.1
    21           5075        0.031     155      25.6%       6.1

Число сделок удвоилось, а качество упало вчетверо. Механизм понятен и он не
про риск: кандидаты отсортированы по силе, лучшие идут первыми, и предел
работал ФИЛЬТРОМ КАЧЕСТВА. Сетапы с шестого по двадцать первый систематически
хуже первых пяти.

На медвежьем периоде наоборот — без предела 233 R против 150, но просадка там
34% в обоих случаях.

РЕШЕНИЕ ОПЕРАТОРА (2026-08-07): ОСТАВИТЬ БЕЗ ПРЕДЕЛА. Рекомендация вернуть
Фибоначчи пятёрку была высказана вместе с этими числами и отклонена. Мы на
бумажном тестировании, где ценность наблюдений выше доходности кривой, а
бюджет назначается вручную. Вопрос закрыт и переоткрытию по своей инициативе
не подлежит — предел меняется только полем на панели.
...
# Шестёрка в списке не для красоты: это нынешний предел у стратегии уровней.
# Без неё сводная таблица не находила базу для сравнения и показывала по всей
# строке прочерки — замер отработал, а ответа по уровням не дал.
CAPS = (3, 5, 6, 8, 12, 16, 21)
BOTH = ('LONG', 'SHORT')
