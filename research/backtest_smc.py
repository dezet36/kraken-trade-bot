...
def fibo_orders(pair, data, reasons=None):
    ...
    НЕ ПОЛНАЯ МОДЕЛЬ БОЯ (26.09.2026). Здесь нет проверки сканера «цена ещё не
    откатила глубже 38.2%» (pair_scanner, W11), нет смещения лимита 0.1% и
    предела издержек 5%, а сигнал считается по ЗАКРЫТОЙ свече, тогда как
