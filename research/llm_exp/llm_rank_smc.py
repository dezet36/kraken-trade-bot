...
INTRO = {
    'C': (
        'Ты — трейдер-аналитик smart money. Ниже 10 сетапов одной стратегии: лимитный вход от нетронутого '
        'ордер-блока в дисконте (для лонга) или премии (для шорта) по направлению старшего тренда; стоп за краем '
        'блока; цели — расширения Фибоначчи, далеко, в несколько R. Названия монет и даты скрыты. '
        'Все проценты хода даны В СТОРОНУ СДЕЛКИ: плюс — цена уже шла туда, куда сделка. '
        'Оцени для каждого сетапа вероятность в процентах, что сделка закроется в плюс (цена дойдёт хотя бы до '
        'первой цели раньше стопа). У таких сетапов в среднем это около 20–25%, и она сильно зависит от условий — различай их.'
    ),
    'A': (
...
def card_line(c):
    ...
    return (f"{c['id']}: {c['side']} | вход {c['entry_away']:.2f}% от цены, стоп {c['stop']:.2f}%, цель {c['rr']:.2f}R | "
            ...
            + (f" | конфлюенс {c['conf']:.1f} (порог 4.5)" if c.get('conf') is not None else ''))
