...
РЕЗУЛЬТАТ, 2026-08-06. ПЕРЕКОС ЧАСТНЫЙ — он про Фибоначчи, а не про рынок.

    стратегия      бык 2025-26                медведь 2022-23
    FIBO      +0.104 [+0.037; +0.173]    +0.092 [+0.034; +0.150]   ШОРТЫ СИЛЬНЕЕ
    SMC       +0.357 [-0.450; +1.151]    -0.190 [-0.808; +0.405]
    LEVELS    +0.283 [-0.242; +0.829]    +0.193 [-0.335; +0.722]

Только у Фибоначчи разница устойчива и её интервал не накрывает ноль на
обоих периодах. Значит общего правила «в крипте работают шорты» вводить
нельзя, и решение остаётся там, где уже принято, — на одной стратегии.

ГЛАВНОЕ ЗДЕСЬ ДАЖЕ НЕ ЭТО. У SMC разница МЕНЯЕТ ЗНАК между периодами:
+0.357 на растущем рынке и -0.190 на падающем, оба интервала шириной больше
единицы. Перекоса, который мы объясняли четырьмя гипотезами и две из них
успели отвергнуть, попросту НЕ СУЩЕСТВУЕТ: это шум на двух сотнях сделок.
Объяснять было нечего.

Стоит запомнить как приём: прежде чем искать причину явления, надо проверить
устойчиво ли само явление. Две отвергнутые гипотезы — это работа, потраченная
на объяснение случайности, и остановить её могла одна такая таблица.

У SMC и уровней сделок в разы меньше (204 и 293 против 1917 у Фибоначчи), и
интервалы соответственно шире. Отсутствие доказанной разницы у них — не
доказательство её отсутствия, а нехватка наблюдений. Но и действовать по
недоказанному нельзя.
    ...
    for label, (data, smc_period) in periods.items():
        ...
        try:
            ...
            books = {
                ...
                'LEVELS': (orders_levels(data, pairs), exec_5m,
