...
РЕЗУЛЬТАТ, 2026-08-07. КРАЯ НЕТ, И ПРИЧИНА ВИДНА ТОЧНО.

    вариант                       RR   винрейт   валовый бык / медведь
    возврат быстрее (2 бара)    2.90   27-31%      -0.050 / +0.071
    прокол глубже (0.5 ATR)     2.51   29-31%      -0.108 / +0.005
    вход лимитом (мейкер)       2.75   27-30%      -0.134 / +0.007
    цель до середины            1.63   33-34%      -0.164 / -0.131

Ни один вариант приёмку не проходит. Валовый край МЕНЯЕТ ЗНАК между
периодами: на медвежьем около нуля и чуть выше, на бычьем отрицателен.
Устойчивого края нет — есть режимная зависимость, которую двусторонняя
приёмка для того и ставилась.

ПОПРАВКА 2026-08-08: ПРО РЕЖИМНУЮ ЗАВИСИМОСТЬ НАПИСАНО НЕВЕРНО. Оба периода
здесь позже оказались падающими (BTC −34.7% и −39.8%), так что «бык» и
«медведь» были ярлыками, а не измерением, и вывод о зависимости от режима не
на чем было строить. Перепроверка на четырёх периодах, включая два подлинно
растущих, — research/range_sweep_four.py — показала одинаковую картину во всех
четырёх: восемь вариантов из девяти отрицательны везде. Режим ни при чём, край
просто отрицателен, а «смена знака» была тем, как выглядит шум в двух точках.
Само закрытие темы при этом подтвердилось.

ГЕОМЕТРИЯ ПРИ ЭТОМ ИСПРАВНА, И ЭТО ВАЖНО. Отношение риска к прибыли 2.5-2.9
против 0.38-0.60 у сетки — цель на противоположном крае сделала своё дело.
Безубыток при RR 2.8 требует 26.3% попаданий, измерено 26.7-30.8%. То есть
сетап стоит ровно на линии подбрасывания монеты для своей геометрии: форма
починена, предсказательной силы нет.

ПРО ОБЪЁМ ОТВЕТА НЕ ПОЛУЧИЛОСЬ. Вайкоффское условие (тихий прокол) оставило
30 и 33 сделки — на таком числе интервал шире единицы, вывод невозможен. Наше
условие (громкий возврат) дало -0.121 и -0.187, то есть не помогло. Оба
утверждения остались непроверенными по существу.

ЧТО ЗАКРЫВАЕТ ЭТОТ ЗАМЕР. Тему торговли в коридоре целиком, тремя разными
входами: на пробое границы, лимитом на границе, после возврата внутрь. Все
три на одних и тех же данных, все три без края. Горизонтальный коридор,
построенный по недавней истории, на часовом горизонте не даёт преимущества
ни в одну сторону.

Отдельно стоит отметить, чего это НЕ отменяет: стратегия уровней, работающая
на том же принципе прокола с возвратом, край имеет (0.279 и 0.374 R). Разница
в том, что она ищет уровень по касаниям за месяц, а не границу коридора за
двое суток. Значит дело не в приёме, а в том, какой уровень считать значимым.
